    ...
    if (!email) {
      ...
      return res
        ...
        .json({
          success: false,
          message:
            "Email is not available in session. Please restart the process.",
        });
    }
...
const changeEmailValid = async (req, res) => {
  try {
    ...
    if (userExists) {
      ...
      if (emailSent) {
        req.session.userOtp = otp;
        req.session.userData = { email, name: userExists.name }; 
        console.log("Email stored in session:", req.session.userData);

        res.render("change-email-otp", { email }); 
...
const verifyEmailOtp = async (req, res) => {
  try {
    ...
    if (String(enteredOtp) === String(req.session.userOtp)) {
      ...
      console.log(
        "Session userData before rendering new-email:",
...
const setDefaultAddress = async (req, res) => {
  try {
    ...
    const addressId = req.body.addressId; // Address ID from request body
